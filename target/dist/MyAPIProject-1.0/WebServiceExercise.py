...
common_header = {
        'Authorization': "Bearer "+config['Auth']['Authorization']
}
...
def quotes_by_movie_and_character(movieId, characterId):
    ...
    if(movieId in movies_dict and characterId in characters_dict):
        ...
        # If the response status for all URLs are 200 then only send the desired output, else send the Bad Request as Response
...
def getAPIResponse(movieId,characterId):
